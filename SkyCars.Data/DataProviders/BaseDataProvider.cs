...
namespace SkyCars.Data.DataProviders
{
    public abstract class BaseDataProvider
    {
        ...
        private static void UpdateOutputParameters(DataConnection dataConnection, DataParameter[] parameters)
        {
            ...
            foreach (var dataParam in parameters.Where(p => p.Direction == ParameterDirection.Output))
            {
        ...
        protected virtual async Task<DataConnection> CreateLogDataConnectionAsync(LinqToDB.DataProvider.IDataProvider dataProvider)
        {
            ...
            var dataContext = new DataConnection(dataProvider, await CreateDbConnectionAsync(DataSettingsManager.LoadSettings().LogConnectionString), GetMappingSchema())
            {
                CommandTimeout = await DataSettingsManager.GetSqlCommandTimeoutAsync()
            };
        ...
        {
            using var dataContext = await CreateDataConnectionAsync();
            var command = new CommandInfo(dataContext, sql, parameters);
        ...
        public virtual async Task<T> ExecuteNonQueryAsync<T>(string sql, params DataParameter[] parameters)
        {
            using var dataContext = await CreateDataConnectionAsync();
            var command = new CommandInfo(dataContext, sql, parameters);
            var affectedRecords = await command.ExecuteAsync<T>();
            UpdateOutputParameters(dataContext, parameters);
            return affectedRecords;
        }
        ...
        /// The task result contains the queryable source
        /// </returns>
        public virtual async Task<ITable<TEntity>> GetLogTableAsync<TEntity>() where TEntity : BaseEntity
        {
            return new DataContext(LinqToDbDataProvider, await GetLogConnectionStringAsync()) { MappingSchema = GetMappingSchema() }
                .GetTable<TEntity>();
        }
        ...
        /// <typeparam name="TEntity">Entity type</typeparam>
        /// <returns>Queryable source</returns>
        public virtual ITable<TEntity> GetLogTable<TEntity>() where TEntity : BaseEntity
        {
            return new DataContext(LinqToDbDataProvider, GetLogConnectionString()) { MappingSchema = GetMappingSchema() }
                .GetTable<TEntity>();
        }
        ...
        {
            using var dataContext = await CreateLogDataConnectionAsync(LinqToDbDataProvider);
            entity.Id = await dataContext.InsertWithInt32IdentityAsync(entity);
            return entity;
        }
